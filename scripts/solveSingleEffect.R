...
eqNormPrev <- function(L,mu,bt,Ne,h2,cost){
    1 - pnorm(dnorminv(0.5*cost^-1 * sqrt(L*mu*bt*log((1+bt)/(1-bt))/(Ne*h2) )))
}
...
PoissonRiskDiffThr <- function(t,mean,env.sd,alphal=1,this.pi,bigGamma=1,rcv=FALSE,verboseOutput=FALSE){
    if(rcv)
        recover()
    prot <- pPoisConv(t,mean,env.sd,alphal,risk.allele=FALSE)
    risk <- pPoisConv(t,mean,env.sd,alphal,risk.allele=TRUE)
...
SolvePoissonGivenThr <- function(myTheta,thr,fitCost,Ne,L,h2,alphal=1,allow.small.effect=FALSE,verbose.output=FALSE){
